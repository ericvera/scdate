...
it('returns undefined when rules have overlapping weekdays but different times', () => {
  const rule1: WeeklyScheduleRule = {
    weekdays: 'SMTWTFS',
    times: [{ from: '09:00', to: sTime('12:00') }],
  }

  const rule2: WeeklyScheduleRule = {
    weekdays: sWeekdays('SMTWTFS'),
    times: [{ from: sTime('13:00'), to: '17:00' }],
  }
...
it('detects overlap when rules have same weekdays and overlapping times', () => {
  ...
  const rule2: WeeklyScheduleRule = {
    ...
    times: [{ from: sTime('14:00'), to: '18:00' }],
  }
...
it('detects overlap with multiple time ranges', () => {
  const rule1: WeeklyScheduleRule = {
    // Monday
    weekdays: '-M-----',
    times: [
      { from: '09:00', to: sTime('12:00') },
      { from: sTime('13:00'), to: '17:00' },
    ],
  }
  ...
  const rule2: WeeklyScheduleRule = {
    // Monday
    weekdays: sWeekdays('-M-----'),
    times: [{ from: sTime('11:00'), to: '14:00' }],
  }
